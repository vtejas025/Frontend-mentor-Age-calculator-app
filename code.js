...
function displayErrorMessage(ele,value){
...
function isInPast(){
    ...
    if(Number(year.value) > date.getFullYear()){
        ...
        if(Number(month.value) - 1 > date.getMonth()){
            if(monthValid){
                ...
                monthPast=true;
                errorMessage="Must be in the past";
                displayErrorMessage(month,true);
    ...
    if(!yearPast && yearValid){
        displayErrorMessage(year,false);
    }
    if(!monthPast && monthValid){
        displayErrorMessage(month,false);
    }
    if(!dayPast && dayValid){
        displayErrorMessage(day,false);
    }
...
main.addEventListener("focusin",(e)=>{
    if(e.target.matches("input")){
        ...
        displayErrorMessage(e.target,false);
    }
});
