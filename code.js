...
form.addEventListener("submit",ele=>{
    ...
    if(!haserror.has(year.id) && Number(month.value)<=date.getMonth()+1 && Number(day.value)>date.getDate()){
        if(!haserror.has(day.id)){
    ...
    if(haserror.size===0){
        ...
        output.children[0].children[0].textContent=" "+y+" ";
        output.children[1].children[0].textContent=" "+m+" ";
        output.children[2].children[0].textContent=" "+d+" ";
    }
});
